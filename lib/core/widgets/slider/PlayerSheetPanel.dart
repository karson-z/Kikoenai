...
class PlayerSheetPanel extends StatefulWidget {
  final Widget? panel;
  final Widget Function(ScrollController sc, AnimationController controller)?
  panelBuilder;
  ...
  const PlayerSheetPanel({
    ...
    this.boxShadow = const <BoxShadow>[
      BoxShadow(
        ...
        offset: Offset(0, -10), // Y 轴负值将阴影向上偏移，完美避开下方
      )
    ],
  ...
  @override
  _PlayerSheetPanelState createState() => _PlayerSheetPanelState();
}

class _PlayerSheetPanelState extends State<PlayerSheetPanel>
            ...
            : AnimatedBuilder(
                ...
                builder: (context, child) {
                  return Positioned(
                    left: 0.0,
                    right: 0.0,
                    // 【核心修改】：动态控制面板的 bottom 偏移量
                    // 当 _ac.value 为 0 (折叠) 时，bottom 为底部导航栏的高度，恰好坐在导航栏上面不遮挡。
                    // 当 _ac.value 为 1 (展开) 时，bottom 为 0，面板完全沉底，占据刚刚隐藏的导航栏空间。
                    bottom:
                        (widget.bottomNavBar != null && widget.showBottomNavBar)
                        ? widget.bottomNavBarHeight * (1.0 - _ac.value)
                        : 0.0,
  ...
  // 这样 PlayerSheetPanelPanel 的手势和子组件的手势会进入"竞技场"竞争。
...
class PanelController {
  _PlayerSheetPanelState? _panelState;

  void _addState(_PlayerSheetPanelState panelState) {
  ...
  Future<void> close() {
    assert(isAttached, "PanelController must be attached to a PlayerSheetPanelPanel");
  ...
  Future<void> animatePanelToSnapPoint({
    ...
    assert(
      ...
      "PlayerSheetPanelPanel snapPoint property must not be null",
    );
