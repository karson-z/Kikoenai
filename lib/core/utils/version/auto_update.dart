...
class AutoUpdater {
  ...
  Future<List<InstallationType>> _detectAvailableInstallationTypes() async {
    ...
    try {
      ...
      KikoenaiLogger().w('Update: detect installation types failed', error: e);
    }
  ...
  Future<UpdateInfo?> checkForUpdates() async {
    try {
      final response = await _dio.get(VersionConfig.latestApp);
      ...
      KikoenaiLogger().e('Update: check for updates failed', error: e);
      rethrow;
    }
  }
  ...
  Future<void> autoCheckForUpdates() async {
    final autoUpdate =
    setting.get(StorageKeys.autoUpdate, defaultValue: true);
    ...
    try {
      ...
      // 自动检查失败时不显示错误
      KikoenaiLogger().w('Update: auto check for updates failed', error: e);
    }
  }
  ...
  Future<void> manualCheckForUpdates() async {
    try {
      ...
      if (updateInfo != null) {
        ...
        KikoenaiToast.info('当前已经是最新版本！');
      }
    } catch (e) {
      KikoenaiToast.error('检查更新失败');
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                if (updateInfo.publishedAt.isNotEmpty) ...[
                  ...
                  Text(
                    '发布时间: ${OtherUtil.formatDate(updateInfo.publishedAt)}',
    ...
    try {
      ...
      if (selectedType == InstallationType.ios ||
        ...
        if (releaseUrl.isEmpty) {
          releaseUrl = VersionConfig.latestApp;
        }
      ...
      if (downloadUrl.isEmpty) {
        KikoenaiToast.error('没有找到 ${_getInstallationTypeDescription(selectedType)} 的下载链接');
        return;
      }
      ...
      KikoenaiToast.error('下载失败: ${e.toString()}');
      KikoenaiLogger().e('Update: download update failed', error: e);
    }
  }
  ...
  Future<void> _downloadUpdate(
    ...
    if (updateInfo.downloadUrl.isEmpty) {
      KikoenaiToast.error('没有找到合适的下载链接');
      return;
    }
    ...
    if (await file.exists()) {
      try {
        ...
        if (localHash == expectedHash) {
          // 文件已存在且哈希匹配，直接返回
          KikoenaiLogger().i('Update: file already exists and hash verified, skipping download: $filePath');
        ...
        // 验证过程中出错，删除文件重新下载
        KikoenaiLogger().w('Update: file verification failed, deleting and re-downloading', error: e);
        if (await file.exists()) {
          await file.delete();
        }
      }
  ...
  void _installUpdate(
    ...
    try {
      // 显示准备退出的提示
      KikoenaiToast.warning('准备安装更新，应用即将退出...');
      ...
      if (Platform.isWindows) {
        ...
        if (result.type != ResultType.done) {
          KikoenaiToast.error('无法打开安装文件: ${result.message}');
          return;
        }
      }
    } catch (e) {
      KikoenaiToast.error('启动安装程序失败: ${e.toString()}');
      KikoenaiLogger().e('Update: launch installer failed', error: e);
    }
  }
  ...
  void _revealInFileManager(String filePath) async {
    try {
      ...
      KikoenaiToast.error('无法打开文件管理器');
      KikoenaiLogger().w('Update: reveal in file manager failed', error: e);
    }
  }
