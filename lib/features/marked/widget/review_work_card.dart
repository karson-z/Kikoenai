...
import '../../album/widget/rating_section.dart';
import '../../album/widget/review_bottom_sheet.dart'; // 引入 BottomSheet
...
class WorkListCard extends ConsumerWidget {
  ...
  const WorkListCard({super.key, required this.work});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    // 3. build 方法增加 ref 参数
    final canOpenDetail = ref.watch(
      surfaceAvailableProvider(AppSurface.remoteAlbumDetailPage),
    );
    final canSubmitReview = ref.watch(
      surfaceAvailableProvider(AppSurface.submitReviewAction),
    );
    // 提取声优名称，如果有多个用逗号连接
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      ...
                      child: RatingSection(
                        ...
                        // 2. 动态数据
                        userRating: (work.userRating is int)
                            ? work.userRating
                            : 0,
                        ...
                        onRatingUpdate: canSubmitReview
                            ? (int newRating) {
                                final currentStatus = UserWorkStatus(
                                  workId: work.id,
                                );
