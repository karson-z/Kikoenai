...
class AlbumPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final canSearch = availableSurfaces.contains(AppSurface.searchPage);
    final canShowPopular = availableSurfaces.contains(
      AppSurface.homePopularSection,
    );
    final canShowRecommended = availableSurfaces.contains(
      AppSurface.homeRecommendedSection,
    );
    final canShowNewest = availableSurfaces.contains(
      AppSurface.homeNewestSection,
    );
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          await Future.wait(<Future<void>>[
            if (canShowPopular)
              ref.refresh(hotWorksProvider.future).then((_) {}),
            if (canShowRecommended)
              ref.refresh(recommendedWorksProvider.future).then((_) {}),
            if (canShowNewest)
              ref.refresh(newWorksProvider.future).then((_) {}),
          ]);
        },
        child: CustomScrollView(
          slivers: [
            if (isEmpty) // 全局空状态占位符
              ...
              if (canShowPopular) const _HotWorksSection(),
              if (canShowRecommended) const _RecommendedWorksSection(),
              if (canShowNewest) const _NewestWorksSection(),
            ],
          ],
        ),
      ),
    ...
    return hotState.when(
            ...
            SectionHeader(
              ...
              onMore: () {
                context.push(
                  ...
                  extra: {'title': '热门作品', 'source': WorkDataSource.hot},
                );
              },
            ),
            SliverToBoxAdapter(
      ...
      loading: () => const SliverMainAxisGroup(
        slivers: [
          ...
          SliverToBoxAdapter(child: _SectionLoadingBox(message: '热门作品加载中...')),
        ],
      ),
    ...
    return recommendedState.when(
      ...
      loading: () => const SliverMainAxisGroup(
        slivers: [
          ...
          SliverToBoxAdapter(child: _SectionLoadingBox(message: '推荐作品加载中...')),
        ],
      ),
      error: (error, _) => SliverMainAxisGroup(
        slivers: [
    ...
    return newestState.when(
      ...
      loading: () => const SliverMainAxisGroup(
        slivers: [
          ...
          SliverToBoxAdapter(
            child: _SectionLoadingBox(message: '最新作品加载中...', minHeight: 260),
          ),
        ],
      ),
      error: (error, _) => SliverMainAxisGroup(
  ...
  Widget build(BuildContext context) {
    return SliverToBoxAdapter(
      child: SizedBox(
        height: 120,
        child: Center(
          child: Text('加载失败: $error', style: TextStyle(color: Colors.red[300])),
        ),
      ),
    );
  }
...
class _SectionLoadingBox extends StatelessWidget {
  ...
  const _SectionLoadingBox({required this.message, this.minHeight = 180});
  @override
  Widget build(BuildContext context) {
    return SizedBox(
      height: minHeight,
      child: Center(child: LottieLoadingIndicator(size: 72, message: message)),
    );
  }
}
