...
class HandleSubmit {
  ...
  static Future<void> handleRatingSubmit(
    ...
    try {
      // 2. 获取 API 客户端
      final String targetSiteId = siteId ?? ref.read(activeSiteIdProvider);
      final api = ref.read(siteApiByIdProvider(targetSiteId));
      if (!api.supports(SiteFeature.submitReview)) {
        throw UnsupportedError('当前站点不支持提交评价');
      }
      await api.submitReview(
        workId: remoteId ?? newStatus.workId.toString(),
