...
import 'package:kikoenai/core/widgets/filter/filter_panel.dart';
...
import 'package:kikoenai/core/widgets/filter/provider/site_option_providers.dart';
...
void main() {
  testWidgets(
    'filter dropdown panel keeps the site-backed default data source',
    (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: const MaterialApp(
            home: Scaffold(
              body: FilterPanelContainer(module: FilterModule.category),
            ),
          ),
        ),
      );
  ...
  testWidgets(
    'DL library keeps the collapsed bar and opens the dropdown panel',
    (tester) async {
      await tester.binding.setSurfaceSize(const Size(430, 900));
      addTearDown(() => tester.binding.setSurfaceSize(null));
      ...
      expect(find.byType(FilterPanelContainer), findsNothing);
      ...
      final filterPanel = find.byType(FilterPanelContainer);
