...
mixin LyricLayoutMixin<T extends StatefulWidget> on State<T> {
  ...
  void onStyleChange() {
    ...
    if (comparison == RenderComparison.paint) {
      layout?.metrics.forEach((element) {
        element.textPainter.text = TextSpan(
          text: element.line.text,
          style: newStyle.textStyle,
        );
        element.activeTextPainter.text = TextSpan(
          text: element.line.text,
          style: newStyle.activeStyle,
        );
        ...
        element.translationTextPainter.text = TextSpan(
          text: element.line.translation,
          style: newStyle.translationStyle,
        );
      });
      setState(() {});
    }
  ...
  void computeLyricLayout() {
    ...
    final computedLayout = LyricLayout.compute(lyricModel, style, lyricSize);
