...
class LyricPainter extends CustomPainter {
  ...
  void drawHighlight(
    ...
    for (var line in metrics) {
      ...
      if (fadeWidth > 0 && fadeStart < line.width) {
        ...
        addSegment(
          fadeRect,
          LinearGradient(
            colors: [opColors.last, fadeEndColor],
    ...
    if (segments.isNotEmpty && layerBounds != null) {
      final glowRadius = style.activeLineGlowRadius;
      final glowOpacity = style.activeLineGlowOpacity;
      if (glowRadius > 0 && glowOpacity > 0) {
        final glowBounds = layerBounds!.inflate(glowRadius * 2);
        canvas.saveLayer(
          glowBounds,
          Paint()
      ...
      _drawMaskedHighlightSegments(canvas, maskPainter, layerBounds!, segments);
    }
  }
  ...
  double _resolveInactiveBlur(int index, bool isActive, bool isInAnchorArea) {
    if (isSelecting && isInAnchorArea) return 0;

    final blurRadius = style.inactiveLineBlurRadius;
    if (blurRadius <= 0 || !style.enableSwitchAnimation) {
      return isActive ? 0 : blurRadius;
    }
    if (index == switchState.enterIndex) {
      return blurRadius * (1 - switchState.enterAnimationValue);
    }
    if (index == switchState.exitIndex) {
      return blurRadius * switchState.exitAnimationValue;
    }
    return isActive ? 0 : blurRadius;
  }
  ...
  void drawLine(
    ...
    final layoutStyle = layout.style;
    final inactiveBlur = _resolveInactiveBlur(index, isActive, isInAnchorArea);
    if (inactiveBlur > 0) {
      // Exit animation starts at the active size, so the blur layer must keep
      // the larger bounds even though the line is already marked inactive.
      final lineHeight = layout.getLineHeight(true, index);
      final blurPadding = inactiveBlur * 3;
      canvas.saveLayer(
        Rect.fromLTWH(
          -blurPadding,
          -blurPadding,
          size.width + blurPadding * 2,
          lineHeight + blurPadding * 2,
        ),
        Paint()
          ..imageFilter = ui.ImageFilter.blur(
            sigmaX: inactiveBlur,
