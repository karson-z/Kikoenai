...
class LyricLayout {
  ...
  double lineOffsetY(
    ...
    for (var i = 0; i < metrics.length; i++) {
      ...
      if (i >= index) {
        final anchorOffset = anchorOffsetY(
          i,
          activeIndex == i,
          lineHeight,
          alignment,
  ...
  double anchorAdjustmentOffsetY(int index, int activeIndex) {
    ...
    var anchorOffset = anchorOffsetY(
      index,
      isHighlight,
      lineHeight,
  ...
  factory LyricLayout.updatePainters(LyricLayout layout) {
    ...
    for (var line in layout.metrics) {
      ...
      final words = _calcWordMetrics(
        line.line,
        line.textPainter,
        line.activeTextPainter,
        line.translationTextPainter,
      );
      lineMetrics.add(
        line.copyWith(
  ...
  factory LyricLayout.compute(
    ...
    for (var line in model.lines) {
      ...
      activeTextPainter.text = TextSpan(
        text: line.text,
      ...
      final activeMaskPainter = createHighlightMaskPainter(
        activeTextPainter,
        maxWidth,
      );
