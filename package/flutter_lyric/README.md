...
### Apple Music inspired preset

Use `LyricStyles.appleMusic` for the bundled left-aligned presentation. It adds
spring-based vertical tracking, an active-line scale transition, progressive
word illumination, subtle highlight bloom, and depth-of-field blur on inactive
lines. The preset remains fully compatible with selection, translation lyrics,
and `copyWith` typography overrides.
