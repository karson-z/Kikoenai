...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    public void onCreate() {
        // 获取已缓存的 FlutterEngine。
        FlutterEngine flutterEngine = FlutterEngineCache.getInstance().get(OverlayConstants.CACHED_TAG);

        if (flutterEngine == null) {
            // 如果找不到引擎，则创建一个新的引擎作为兜底。
    ...
    private class TrayAnimationTimerTask extends TimerTask {
        ...
        public TrayAnimationTimerTask() {
            ...
            switch (WindowSetup.positionGravity) {
                case "auto":
                    ...
                    mDestY = autoVerticalDestination(params);
                    return;
                case "left":
                    mDestX = 0;
                    return;
        ...
        private int autoVerticalDestination(WindowManager.LayoutParams params) {
            int availableDistance = Math.max(0, szWindow.y - flutterView.getHeight());
            int verticalGravity = params.gravity & Gravity.VERTICAL_GRAVITY_MASK;
            int minY = 0;
            int maxY = availableDistance;

            if (verticalGravity == Gravity.CENTER_VERTICAL) {
                minY = -(availableDistance / 2);
                maxY = availableDistance / 2;
            }

            return Math.max(minY, Math.min(params.y, maxY));
        }
